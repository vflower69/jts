...
// Pins setup (Triangle formation standard 10-pin)
let pins = [];

function initPins() {
    pins = [];
    const pinRadius = 12;
    // Row configuration (from back row 4 pins to front row 1 pin)
    const pinRows = [
...
canvas.addEventListener('mouseup', () => {
    ...
    if (dy < -5) { // Flick upwards
        launchBall(dx * 0.08, dy * 0.09);
    }
});
...
function gotoMenu() {
    ...
    overlay.querySelector('p').textContent = 'High-octane arcade bowling! Swipe, drag, or use A/D keys to position and shoot with curve physics.';
...
function update(dt) {
    // Keyboard ball positioning during aiming state
    if (gameState === 'AIMING') {
        if (input.keys['KeyA'] || input.keys['ArrowLeft']) {
            ball.x = Math.max(350, ball.x - 4);
        }
        if (input.keys['KeyD'] || input.keys['ArrowRight']) {
            ball.x = Math.min(550, ball.x + 4);
        }
    }
    ...
    if (gameState === 'ROLLING') {
        ...
        // Slight curve effect if sliding
        ball.vx *= 0.992;

        // Alley boundaries
        if (ball.x < 300 || ball.x > 600) {
            ball.vx *= -0.5; // gutter bounce
        }
        ...
        pins.forEach(pin => {
            if (pin.standing) {
                ...
                if (dist < ball.radius + pin.radius) {
                    pin.standing = false;
                    pin.fallen = true;
                    pin.vx = ball.vx * 0.7 + (Math.random() - 0.5) * 4;
                    pin.vy = ball.vy * 0.7 + (Math.random() - 0.5) * 4;
        ...
        pins.forEach((p1, i) => {
            if (p1.fallen && !p1.dead) {
                ...
                if (p1.y < 90 || p1.y > 520 || p1.x < 280 || p1.x > 620) {
                    p1.dead = true;
                }
            }
        });

        // End of roll check when ball passes pins or stops
...
function endThrow() {
    let newlyFallen = pins.filter(p => p.fallen && !p.counted).length;
    // Count standing pins
...
function showGameOver() {
    ...
    overlay.querySelector('p').textContent = `Final Score: ${totalSessionScore(totalScore)}! Outstanding arcade bowling performance!`;
    document.getElementById('start-btn').textContent = 'Play Again';
    document.getElementById('exit-btn').style.display = 'inline-block';
}

function totalSessionScore(score) {
    return score;
}
...
function render() {
    ...
    // --- AIMING DRAG LINE ---
    if (input.dragging && gameState === 'AIMING') {
        ctx.strokeStyle = '#3fb950';
        ctx.lineWidth = 4;
        ctx.setLineDash([6, 6]);
        ctx.beginPath();
        ctx.moveTo(ball.x, ball.y);
        let targetX = ball.x + (input.currX - input.startX);
        let targetY = ball.y + (input.currY - input.startY) * 2;
